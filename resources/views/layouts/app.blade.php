...
<body class="bg-gray-50 font-family-karla">
    ...
    <nav class="w-full py-4 border-t border-b bg-gray-100" x-data="{ open: false }">
        ...
        <div :class="open ? 'block' : 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
            <div class="w-full mx-auto flex ml-4 flex-col sm:flex-row text-sm font-bold uppercase mt-0 px-6 py-2">
                    ...
                    <div class="flex items-center mr-20">
                        <form method="get" action="{{ route('search') }}">
                            <input name="q" value="{{ request()->get('q') }}"
                                ...
                                :rows="isEdit || focused ? '2' : '1'" placeholder="Type to search anything" />
                        </form>
                        @auth
                            <div class="flex sm:items-center sm:ml-6">
                                <x-dropdown align="right" width="48">
                                    <x-slot name="trigger">
                                        <button class="hover:bg-blue-600 flex items-center hover:text-white rounded py-2 px-4 mx-2">
